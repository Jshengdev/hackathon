...
export const NOTES: Record<string, string> = {
  ...
  "swarm-architecture":
    "the swarm in one picture. video splits at the top — qwen3-vl baseline + tribe v2 multimodal brain encoder. eight k2 specialists fire in parallel at ~1,300 tokens per second, each one assigned to a section of the activation JSON, simulating how its region reacts and mapping back via cosine similarity to the actual tribe v2 numbers. iterative loop runs INSIDE stage 2 — 2 to 3 rounds, score climbs from 0.70 to 0.85, plateau exit. moderator synthesizes the eight readings + vision into one observational document. quick falsification verifies the signal is anchored to the scene — important for the data-ingestion stage. final output: brain-grounded paragraph + interrogable amy chat. reports surface what humans saw, what they experienced, current stress levels — semantic readings inferred from cortical patterns, not just what they did. tribe v2 open-source on huggingface, cc-by-nc-4.0.",

  "simulation-architecture":
    "this is stage 2 deep-dive — the k2 swarm simulation that does the actual brain interpretation. eight specialist agents fire in parallel, one per cortical region. each one runs its own k2 reasoning chain at about 1,300 tokens per second on cerebras. across 2-3 iterative rounds with plateau exit, we run roughly 24 distinct thought processes per clip — about 25,000 tokens of inference, all under 60 seconds. — the math only closes at k2's throughput. without it, 24 sequential llm calls would take 4 to 5 minutes. and without k2's low hallucination rate, the specialists would drift unreliably from the brain pattern, breaking the cosine similarity scoring entirely. — gpt-class models are too slow at this load. local llms are too noisy. a single-agent approach can't represent distributed brain regions. k2 is the only vendor that closes the loop. — and what the swarm is actually doing is simulating how a brain interprets itself — eight regions of cortex, each with its own perspective, converging on a shared semantic reading via per-region miss-attribution feedback. this is the simulation architecture for the classification of brain data within our pipeline.",

  "website-architecture":
    "what we actually shipped. browser → vue 3 frontend (five stages: landing, loading, main, iterative-reveal, empathy-document) → fastapi backend → internal services (swarm_runner, iterative_loop, empathy_synthesis, empathy_polish, empathy_chat for amy, falsification, vision_client) → pre-rendered data on disk → external apis (k2 think on cerebras, openrouter to qwen3-vl, anthropic to opus 4.7, meta tribe v2 baked offline). runtime never invokes tribe v2 — activity.json is the canonical brain artifact. that's the reproducibility virtue.",
  close:
