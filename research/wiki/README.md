...
- [**Mira**](projects/mira.md) — ★ NEW. TreeHacks 2026 Most Impactful + OpenAI AI Track 1st. Ray-Ban + spatial VLM eldercare. **🔴 HIGH lookalike risk** — owns voice-to-3D + evidence chips. 6 Devpost claims unimplemented (Grounded SAM, MapAnything, ElevenLabs, email, FHIR, hardcoded keys).
- [**Tribune**](projects/tribune.md) — ★ NEW. TreeHacks 2026 Anthropic Human Flourishing 1st. Civic policy diff with cited constituent voices. **🟠 8/10 lookalike risk** — civic-tech + voice + cited-diff combo locked from us; key Devpost claim "audio-clip citations" is unimplemented (text only).
- [**ContainOS**](projects/containos.md) — ★ NEW. TreeHacks 2026 OpenAI AI Track 1st. Wildfire incident command. **🟡 6/10 lookalike risk** — claimed multi-LLM ensemble fires only GPT-4o + Gemini; Claude SDK installed but unused (mocked with hardcoded English).
- [**Keryx**](projects/keryx.md) — ★ NEW. TreeHacks 2026 Human Capital Fellowship. Drone + VLM navigable world model. **🟡 medium lookalike risk** — Apple Depth Pro shipped but never called by agent path; spatial-sidecar wedge wide open.
- [**The Orchestration Co. of Palo Alto**](projects/orchestration-co-of-palo-alto.md) — ★ NEW. TreeHacks 2026 Interaction Co. (Poke). Vision Pro AR control room with agents-as-3D-desks. **🔴 HIGH lookalike risk** — closest precedent for witnessed-dissent UI (with external referent: Vercel preview).
- [**4sight**](projects/4sight.md) — ★ NEW. TreeHacks 2026 Cloudflare Best Use ($250K credits). Wearable biometrics + egocentric VLM + Cloudflare-edge nudge. **🔴 HIGH lookalike risk** — not actually an agent loop (1-min Worker cron over D1 SQL calling gpt-4o-mini).
